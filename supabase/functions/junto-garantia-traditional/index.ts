...
async function authenticate(environment: Environment): Promise<string> {
  ...
  console.log(`[junto-traditional] Token renovado para ${environment}, expira em ${expiresIn}s`);
  return bearer;
}
...
async function juntoFetch(url: string, env: Environment, options: RequestInit = {}): Promise<Response> {
  ...
  if (response.status === 401) {
    console.log("[junto-traditional] 401, forçando refresh...");
    tokenCache[env] = null;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── ACTION: createQuote (POST /traditional) ──
    if (action === "createQuote") {
      ...
      const url = `${baseUrl}/traditional`;
      console.log(`[junto-traditional] Criar Cotação: POST ${url}`);
      ...
      console.log(`[junto-traditional] createQuote status: ${response.status}`);

      if (!response.ok) {
        console.error(`[junto-traditional] Erro createQuote: ${responseText}`);
    ...
    // ── ACTION: updateQuote (PUT /traditional) ──
    if (action === "updateQuote") {
      ...
      const url = `${baseUrl}/traditional`;
      console.log(`[junto-traditional] Atualizar Cotação: PUT ${url}`);
      ...
      const responseText = await response.text();
      console.log(`[junto-traditional] updateQuote status: ${response.status}`);

      if (!response.ok) {
        console.error(`[junto-traditional] Erro updateQuote: ${responseText}`);
        return new Response(JSON.stringify({
          success: false,
    ...
    // ── ACTION: getQuote (GET /traditional/{quoteId}) ──
    if (action === "getQuote") {
      ...
      const url = `${baseUrl}/traditional/${quoteId}`;
      console.log(`[junto-traditional] Consultar: GET ${url}`);

      const response = await juntoFetch(url, environment);
      const responseText = await response.text();
      console.log(`[junto-traditional] getQuote status: ${response.status}`);

      if (!response.ok) {
        console.error(`[junto-traditional] Erro getQuote: ${responseText}`);
        return new Response(JSON.stringify({
          success: false,
    ...
    // ── ACTION: upsertMinuta (PUT /traditional/{quoteId}/draft) ──
    if (action === "upsertMinuta") {
      ...
      const url = `${baseUrl}/traditional/${quoteId}/draft`;
      console.log(`[junto-traditional] Minuta: PUT ${url}`);
      ...
      const responseText = await response.text();
      console.log(`[junto-traditional] upsertMinuta status: ${response.status}`);

      if (!response.ok) {
        console.error(`[junto-traditional] Erro upsertMinuta: ${responseText}`);
        return new Response(JSON.stringify({
          success: false,
    ...
    // ── ACTION: requestEmission (POST /traditional/{quoteId}/emission) ──
    if (action === "requestEmission") {
      ...
      const url = `${baseUrl}/traditional/${quoteId}/emission`;
      console.log(`[junto-traditional] Emissão: POST ${url}`);
      ...
      const responseText = await response.text();
      console.log(`[junto-traditional] requestEmission status: ${response.status}`);

      if (!response.ok) {
        console.error(`[junto-traditional] Erro requestEmission: ${responseText}`);
        return new Response(JSON.stringify({
          success: false,
    ...
    console.error("[junto-traditional] Erro:", error);
    return new Response(JSON.stringify({
      success: false,
