...
async function authenticate(environment: Environment): Promise<string> {
  ...
  console.log(`[junto-judicial-labor-review] Token renovado para ${environment}`);
  return bearer;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── createQuote (POST /judicial-labor-review) ──
    if (action === "createQuote") {
      const payload = body.payload;
      if (!payload) return jsonResponse({ success: false, error: "payload é obrigatório" });

      const url = `${baseUrl}/judicial-labor-review`;
      console.log(`[junto-judicial-labor-review] POST ${url}`);

      const response = await juntoFetch(url, environment, { method: "POST", body: JSON.stringify(payload) });
      const responseText = await response.text();

      if (!response.ok) {
        console.error(`[junto-judicial-labor-review] Erro createQuote: ${responseText}`);
    ...
    // ── updateQuote (PUT /judicial-labor-review) ──
    if (action === "updateQuote") {
      const payload = body.payload;
      if (!payload) return jsonResponse({ success: false, error: "payload é obrigatório" });

      const url = `${baseUrl}/judicial-labor-review`;
      console.log(`[junto-judicial-labor-review] PUT ${url}`);

      const response = await juntoFetch(url, environment, { method: "PUT", body: JSON.stringify(payload) });
      const responseText = await response.text();

      if (!response.ok) {
        console.error(`[junto-judicial-labor-review] Erro updateQuote: ${responseText}`);
    ...
    // ── getQuote (GET /judicial-labor-review/{quoteId}) ──
    if (action === "getQuote") {
      const quoteId = body.quoteId;
      if (!quoteId) return jsonResponse({ success: false, error: "quoteId é obrigatório" });

      const url = `${baseUrl}/judicial-labor-review/${quoteId}`;
      console.log(`[junto-judicial-labor-review] GET ${url}`);

      const response = await juntoFetch(url, environment);
      const responseText = await response.text();

      if (!response.ok) {
        console.error(`[junto-judicial-labor-review] Erro getQuote: ${responseText}`);
    ...
    // ── upsertMinuta (PUT /judicial-labor-review/{quoteId}/draft) ──
    if (action === "upsertMinuta") {
      const quoteId = body.quoteId;
      if (!quoteId) return jsonResponse({ success: false, error: "quoteId é obrigatório" });

      const url = `${baseUrl}/judicial-labor-review/${quoteId}/draft`;
      console.log(`[junto-judicial-labor-review] PUT ${url}`);
      ...
      if (!response.ok) {
        console.error(`[junto-judicial-labor-review] Erro upsertMinuta: ${responseText}`);
    ...
    // ── requestEmission (POST /judicial-labor-review/{quoteId}/emission) ──
    if (action === "requestEmission") {
      const quoteId = body.quoteId;
      if (!quoteId) return jsonResponse({ success: false, error: "quoteId é obrigatório" });

      const url = `${baseUrl}/judicial-labor-review/${quoteId}/emission`;
      console.log(`[junto-judicial-labor-review] POST ${url}`);
      ...
      if (!response.ok) {
        console.error(`[junto-judicial-labor-review] Erro requestEmission: ${responseText}`);
    ...
    console.error("[junto-judicial-labor-review] Erro:", error);
