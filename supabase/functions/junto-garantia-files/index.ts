...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { action = "upload", environment = "sandbox", documentNumber, files } = body;
    const env = (environment === "production" ? "production" : "sandbox") as Environment;
    const baseUrl = BASE_URLS[env];

    // POST /files/{documentNumber}/proposal — multipart file upload
    if (action === "upload") {
      if (!documentNumber) return jsonResponse({ error: "documentNumber é obrigatório" }, 400);
      if (!files || !Array.isArray(files) || files.length === 0) {
        return jsonResponse({ error: "files é obrigatório (array de { name, base64, mimeType })" }, 400);
      }
      ...
      for (const file of files) {
        const binary = Uint8Array.from(atob(file.base64), (c: string) => c.charCodeAt(0));
        const blob = new Blob([binary], { type: file.mimeType || "application/octet-stream" });
        formData.append("Files", blob, file.name);
      }

      const url = `${baseUrl}/files/${documentNumber}/proposal`;
      console.log(`[files] POST ${url} (${files.length} arquivo(s))`);
