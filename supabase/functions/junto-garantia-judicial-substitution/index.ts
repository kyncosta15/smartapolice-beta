...
async function authenticate(environment: Environment): Promise<string> {
  ...
  console.log(`[junto-judicial-substitution] Token renovado para ${environment}`);
  return bearer;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "createQuote") {
      if (!body.payload) return jsonResponse({ success: false, error: "payload é obrigatório" });
      const url = `${baseUrl}/judicial-substitution`;
      const response = await juntoFetch(url, environment, { method: "POST", body: JSON.stringify(body.payload) });
      const responseText = await response.text();
      if (!response.ok) return jsonResponse({ success: false, error: `Erro ao criar cotação (${response.status})`, details: responseText.slice(0, 1000) });
    ...
    if (action === "updateQuote") {
      if (!body.payload) return jsonResponse({ success: false, error: "payload é obrigatório" });
      const url = `${baseUrl}/judicial-substitution`;
      const response = await juntoFetch(url, environment, { method: "PUT", body: JSON.stringify(body.payload) });
      const responseText = await response.text();
      if (!response.ok) return jsonResponse({ success: false, error: `Erro ao atualizar cotação (${response.status})`, details: responseText.slice(0, 1000) });
      return jsonResponse({ success: true, data: JSON.parse(responseText), environment });
    }

    if (action === "getQuote") {
      if (!body.quoteId) return jsonResponse({ success: false, error: "quoteId é obrigatório" });
      const url = `${baseUrl}/judicial-substitution/${body.quoteId}`;
      const response = await juntoFetch(url, environment);
      const responseText = await response.text();
      if (!response.ok) return jsonResponse({ success: false, error: `Erro ao consultar (${response.status})`, details: responseText.slice(0, 1000) });
      return jsonResponse({ success: true, data: JSON.parse(responseText), environment });
    }

    if (action === "upsertMinuta") {
      if (!body.quoteId) return jsonResponse({ success: false, error: "quoteId é obrigatório" });
      const url = `${baseUrl}/judicial-substitution/${body.quoteId}/draft`;
      const response = await juntoFetch(url, environment, { method: "PUT", body: body.payload ? JSON.stringify(body.payload) : undefined });
      const responseText = await response.text();
      if (!response.ok) return jsonResponse({ success: false, error: `Erro ao criar/atualizar minuta (${response.status})`, details: responseText.slice(0, 1000) });
      let data = null;
    ...
    if (action === "requestEmission") {
      if (!body.quoteId) return jsonResponse({ success: false, error: "quoteId é obrigatório" });
      const url = `${baseUrl}/judicial-substitution/${body.quoteId}/emission`;
      const response = await juntoFetch(url, environment, { method: "POST", body: body.payload ? JSON.stringify(body.payload) : undefined });
      const responseText = await response.text();
      if (!response.ok) return jsonResponse({ success: false, error: `Erro ao solicitar emissão (${response.status})`, details: responseText.slice(0, 1000) });
      let data = null;
      try { data = JSON.parse(responseText); } catch { data = responseText; }
      return jsonResponse({ success: true, data, environment });
    }
    ...
    console.error("[junto-judicial-substitution] Erro:", error);
