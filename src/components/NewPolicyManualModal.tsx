  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: policy, error } = await supabase
        ...
        .insert({
          user_id: user.id,
          nome_apolice: formData.nome_apolice,
          nome_segurado: formData.nome_segurado,
          tipo: formData.tipo_beneficio,
          documento: formData.documento,
          documento_tipo: formData.documento_tipo,
          numero_apolice: formData.numero_apolice,
          premio_total: parseFloat(formData.premio_total.replace(/[^\d,]/g, '').replace(',', '.')),
          valor_mensal: parseFloat(formData.valor_mensal.replace(/[^\d,]/g, '').replace(',', '.')),
