...
export function MyPolicies() {
  ...
  const handleRenewalDecision = async (policy: PolicyWithStatus, newStatus: PolicyStatus) => {
    ...
    const updateSuccess = await updatePolicy(policy.id, {
      status: newStatus
    });
    if (updateSuccess) {
      toast({
  ...
  return <div className="space-y-6 p-4 md:p-0">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <Button onClick={() => setShowNewPolicyModal(true)} size="sm" variant="outline" className="gap-2 whitespace-nowrap">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        ...
        const originalPolicy = policies.find(p => p.id === policy.id);
        const installmentsCount = originalPolicy?.quantidade_parcelas || originalPolicy?.installments?.length || 12;
        console.log(`🎯 [MyPolicies-Render] Renderizando ${policy.name} com status: ${policy.status}`);
        return <Card key={policy.id} className="hover:shadow-lg transition-shadow overflow-hidden">
              ...
              <CardContent className="space-y-3">
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-start gap-2">
                    ...
                    <div className="text-right">
                      <p className="text-xs text-gray-500 mb-1">Nova Apólice</p>
                      <p className={`font-medium text-sm whitespace-nowrap ${new Date(policy.expirationDate || policy.endDate) < new Date() ? 'text-red-600' : 'text-gray-900'}`}>
                ...
                <div className="flex gap-1.5 pt-3 border-t justify-end">
                  <Button variant="ghost" size="sm" onClick={() => handleViewPolicy(policy)} className="h-9 w-9 p-0 hover:bg-primary/10" title="Visualizar apólice">
                    <Eye className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => handleDownloadPolicy(policy)} className="h-9 w-9 p-0 hover:bg-primary/10" title="Baixar apólice">
                    <Download className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => handleEditPolicy(policy)} className="h-9 w-9 p-0 hover:bg-primary/10" title="Editar apólice">
                    <Edit className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="sm" onClick={e => handleDeleteClick(e, policy)} className="h-9 w-9 p-0 hover:bg-red-50 hover:text-red-600" title="Deletar apólice" disabled={isDeleting}>
      ...
      {renewalAlert && <RenewalModal policy={renewalAlert.toRenew} onDecision={newStatus => handleRenewalDecision(renewalAlert.toRenew, newStatus)} onClose={renewalAlert.clear} />}

      <InfoModal isOpen={showInfoModal} onClose={() => setShowInfoModal(false)} />

      <NewPolicyManualModal open={showNewPolicyModal} onOpenChange={setShowNewPolicyModal} onSuccess={refreshPolicies} />

      <PolicyDetailsModal isOpen={showDetailsModal} onClose={() => {
      setShowDetailsModal(false);
      setSelectedPolicy(null);
    }} policy={selectedPolicy} onDelete={async policyId => {
      ...
      <PolicyEditModal isOpen={showEditModal} onClose={() => {
      setShowEditModal(false);
      setSelectedPolicy(null);
    }} policy={selectedPolicy} onSave={handleSaveEdit} />
      ...
      if (!open && !isDeleting) {
        ...
        <AlertDialogContent className="max-w-md">
          ...
          <AlertDialogFooter className="gap-2">
            <AlertDialogCancel onClick={handleCancelDelete} disabled={isDeleting} className="border-gray-300">
              Cancelar
            </AlertDialogCancel>
            <AlertDialogAction onClick={handleConfirmDelete} disabled={isDeleting} className="bg-red-600 hover:bg-red-700 focus:ring-red-600 text-white">
              {isDeleting ? <div className="flex items-center gap-2">
                  <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                  Deletando...
                </div> : "🗑️ Deletar Apólice"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
