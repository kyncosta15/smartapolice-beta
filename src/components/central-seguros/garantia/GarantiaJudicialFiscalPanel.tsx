...
import { Loader2, Search, Plus, FileEdit, Send, FileText, Landmark, Copy, CheckCircle2, ArrowLeft } from 'lucide-react';
...
export function GarantiaJudicialFiscalPanel() {
  ...
  const invoke = useCallback(async (action: string, extra: Record<string, any> = {}) => {
    ...
    setRawResponse('');
    try {
      const { data, error } = await supabase.functions.invoke('junto-garantia-judicial-fiscal', {
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-foreground">Judicial Fiscal</h2>
            <p className="text-sm text-muted-foreground">Gerencie cotações, minutas e emissões fiscais</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Card className="cursor-pointer hover:border-primary/50 transition-colors group" onClick={() => setActiveView('criar')}>
            <CardContent className="p-6 text-center space-y-3">
              ...
              <p className="text-xs text-muted-foreground">Nova cotação Judicial Fiscal</p>
            </CardContent>
          </Card>
          ...
          <Card className="cursor-pointer hover:border-primary/50 transition-colors group" onClick={() => setActiveView('emissao')}>
            <CardContent className="p-5 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">Emitir apólice fiscal</p>
              </div>
            </CardContent>
          </Card>
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {activeView === 'consultar' && (
        <Card>
          <CardContent className="p-5 space-y-4">
            <h3 className="font-semibold text-foreground flex items-center gap-2"><Search className="size-4 text-primary" /> Consultar Judicial Fiscal</h3>
      ...
      {activeView === 'criar' && (
        <Card>
          <CardContent className="p-5 space-y-4">
            <h3 className="font-semibold text-foreground flex items-center gap-2"><Plus className="size-4 text-primary" /> Nova Cotação Judicial Fiscal</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
