...
import { Loader2, Search, Plus, FileEdit, Send, FileText, ShieldCheck, Copy, CheckCircle2, AlertTriangle } from 'lucide-react';
...
export function GarantiaTraditionalPanel() {
  ...
  const [cotacaoForm, setCotacaoForm] = useState({
    ...
    durationStart: '', durationEnd: '', modalityId: '', additionalInfo: '',
  ...
  const invoke = useCallback(async (action: string, extra: Record<string, any> = {}) => {
    ...
    setRawResponse('');
    try {
      const { data, error } = await supabase.functions.invoke('junto-garantia-traditional', {
  ...
  const handleCriar = () => {
    ...
    if (cotacaoForm.contractNumber) payload.contractNumber = cotacaoForm.contractNumber;
    if (cotacaoForm.contractDescription) payload.contractDescription = cotacaoForm.contractDescription;
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Gerencie cotações, minutas e emissões tradicionais</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Card className="cursor-pointer hover:border-primary/50 transition-colors group" onClick={() => setActiveView('criar')}>
            <CardContent className="p-6 text-center space-y-3">
              ...
              <p className="text-xs text-muted-foreground">Nova cotação Traditional</p>
            </CardContent>
          </Card>
  ...
  return (
    <div>
      ...
      {activeView === 'criar' && (
        <Card>
          <CardContent className="p-6 space-y-4">
            <h3 className="font-semibold flex items-center gap-2"><Plus className="size-4" />Nova Cotação Traditional</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div><Label className="text-xs">Nº Contrato</Label><Input value={cotacaoForm.contractNumber} onChange={e => handleChange('contractNumber', e.target.value)} placeholder="Número do contrato" /></div>
              ...
              <div><Label className="text-xs">Descrição do Contrato</Label><Input value={cotacaoForm.contractDescription} onChange={e => handleChange('contractDescription', e.target.value)} placeholder="Objeto do contrato" /></div>
              <div>
                <Label className="text-xs">Modalidade</Label>
                <Select value={cotacaoForm.modalityId} onValueChange={v => handleChange('modalityId', v)}>
                  <SelectTrigger><SelectValue placeholder="Selecione" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="1">Licitante / Bid Bond</SelectItem>
                    <SelectItem value="2">Construtor / Performance</SelectItem>
                    <SelectItem value="3">Fornecedor / Supply</SelectItem>
                    <SelectItem value="4">Prestador de Serviço</SelectItem>
                    <SelectItem value="5">Adiantamento de Pagamento</SelectItem>
                    <SelectItem value="6">Aduaneira</SelectItem>
                    <SelectItem value="7">Imobiliária</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
        ...
        <Card>
          <CardContent className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div><Label className="text-xs">Nº Contrato</Label><Input value={cotacaoForm.contractNumber} onChange={e => handleChange('contractNumber', e.target.value)} /></div>
            </div>
